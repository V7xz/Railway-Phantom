...
async function handleSlash(interaction) {
    ...
    if (commandName === "accept") {
        ...
        if (memberToDM) {
            try {
                await memberToDM.send({ embeds: [approveEmbed] });
                dmSuccess = true;
            } catch (err) {
    ...
    if (name.startsWith("order-") || name.startsWith("claimed-")) {
        ...
        if (logCh) {
            ...
            await logCh.send({
                ...
                components: [
                    new ActionRowBuilder().addComponents(
                        new ButtonBuilder().setCustomId(`approve_${msg.channel.id}`).setLabel("Approve").setStyle(ButtonStyle.Success).setEmoji("✅"),
                        new ButtonBuilder().setCustomId(`reject_${msg.channel.id}`).setLabel("Reject").setStyle(ButtonStyle.Danger).setEmoji("❌")
                    )
                ]
            });
        }
